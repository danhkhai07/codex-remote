// Independent review: extends owned author fixture with actual admin PWA and fresh Linux WebKit.
...
try {
  ...
  writeFileSync(join(files, 'index.html'), '<!doctype html><h1>Owned admin PWA shell</h1>')
  writeFileSync(join(files, 'sw.js'), readFileSync('public/sw.js'))
  for (const name of ['manifest.webmanifest','codex-remote.svg','icon-192.png','icon-512.png','icon-maskable-512.png','apple-touch-icon.png','recovery.css']) writeFileSync(join(files,name), 'owned fixture')
  ...
  const link = seed.create({ port: appPort, ttlSeconds: 600 }); seed.close()
  ...
  // Explicit CONNECT proxy maps only our two synthetic hosts, including WebKit.
  const tunnel = createServer((_req,res) => {res.writeHead(403);res.end()})
  tunnel.on('connect',(req,socket,head)=>{
    const target = new URL('https://'+req.url)
    if (![new URL(publicOrigin).host,`p${appPort}.preview.test:${tlsPort}`].includes(target.host)) { socket.destroy(); return }
  ...
  const proxyPort=await listen(tunnel), evidence=[]
  for (const engine of ['chromium','webkit']) {
    const browser = await ({chromium,webkit}[engine]).launch({headless:true,proxy:{server:`http://127.0.0.1:${proxyPort}`},...(engine==='chromium'?{args:['--no-sandbox','--ignore-certificate-errors']}: {})})
    ...
    if (screenshotDir) { mkdirSync(screenshotDir, { recursive: true }); await page.screenshot({ path: resolve(screenshotDir, `review-${engine}.png`) }) }
    if (engine === 'chromium') {
      await page.evaluate(async () => { await navigator.serviceWorker.register('/sw.js'); await navigator.serviceWorker.ready })
      await page.reload(); await page.waitForFunction(() => Boolean(navigator.serviceWorker.controller))
      await page.goto(link.url)
    ...
    evidence.push({ engine, viewport, actualAdminPwaInstalled:engine==='chromium', crossSiteBootstrap: true, reload: true, noOwnerSession: true, fragmentCleared: true, capOnlySentToAdminExchange: true,
      appWorkerSeesOnlyOneUseHandoff: engine === 'chromium', errors: errors.length })
    await context.close(); await browser.close()
  }
  const result={passed:true,tests:evidence,realModelTurns:0,productionAccess:false}; if(process.env.SHARE_SCREENSHOTS)writeFileSync(resolve(process.env.SHARE_SCREENSHOTS,'browser-result.json'),JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result,null,2))
} finally {

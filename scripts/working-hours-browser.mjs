...
try {
 for (const viewport of [{width: 1280, height: 1000}, {width: 390, height: 844}, {width: 320, height: 700}]) {
  ...
  await frame.locator('.bar-column[data-date="'+day+'"]').filter({hasText: '2h 00m'}).waitFor()
  assert.equal(await frame.locator('#today-worked').count(), 0)
  assert.equal(await frame.locator('.bar-column').count(), 7)
  await frame.locator('#edit-date').fill('2026-09-20')
  await frame.locator('#range').selectOption('month')
  assert.equal(await frame.locator('.bar-column[data-date="'+day+'"]').count(), 0)
  await frame.locator('#range').selectOption('30')
  assert.equal(await frame.locator('.bar-column').count(), 30)
  assert.match(await frame.locator('.bar-column').last().getAttribute('aria-label'), /2h 00m/)
  await frame.locator('#range').selectOption('7')
  ...
  await frame.locator('.bar-column').last().filter({hasText: '3h 00m'}).waitFor()
  ...
  now += hour / 2
  await frame.locator('.bar-column').last().filter({hasText: '3h 30m'}).waitFor()
  await frame.locator('#auto-toggle').click()
  await frame.getByRole('button', {name: 'Tiếp tục', exact: true}).waitFor()
  assert.equal(store.read().timer, null); assert.equal(store.read().totals[day], 3.5)
  assert.match(await frame.locator('.bar-column').last().getAttribute('aria-label'), /3h 30m/)
  await frame.locator('#edit-date').fill('2026-09-20')
  await frame.locator('#range').selectOption('month')
  now = Date.parse('2026-09-22T00:00:30+07:00')
  await frame.locator('#cards').filter({hasText: '2026-09-22'}).waitFor()
  assert.equal(await frame.locator('.bar-column[data-date="2026-09-22"]').count(), 0)
  assert.equal(await frame.locator('#edit-date').inputValue(), '2026-09-20')
  assert.equal(await frame.locator('#range').inputValue(), 'month')
  assert.equal(store.read().autoPaused, true)
  await frame.locator('#range').selectOption('all')
  assert.equal(await frame.locator('.bar-column').last().getAttribute('data-date'), '2026-09-22')
  assert.match(await frame.locator('.bar-column').last().getAttribute('aria-label'), /Unknown/)
  ...
  console.log(`PASS ${viewport.width}: chart includes today, no chip, other date/month, midnight rollover, paused/running, page/file-view and pause controls`)
  await context.close()
 }

...
try {
  ...
  const threads = [makeThread('thread-a', 'Convo A', true), makeThread('thread-b', 'Convo B'), makeThread('thread-c', 'Convo C')]
  const app = new CodexAppServer('UNUSED')
  app.request = async (method, params = {}) => {
    nativeCalls.push({ method, id: params.threadId })
    if (method === 'thread/read' && heldRead?.id === params.threadId) {
      const wait = heldRead; heldRead = null; wait.entered(); await wait.gate
    }
  ...
  for (const viewport of [{ width: 1280, height: 900 }, { width: 390, height: 844 }]) {
    ...
    // An explicit manual conversation choice wins over a delayed notification lookup.
    const manual = holdRead('thread-b')
    await click('thread-b'); await manual.started
    if (viewport.width < 760) await page.getByRole('button', { name: 'Open conversations', exact: true }).click()
    await page.locator('.thread-row').filter({ hasText: 'Convo C' }).click(); await title('Convo C')
    await page.locator('#instruction').fill('Manual C draft')
    manual.release()
    await page.waitForFunction(() => location.hash === '')
    await title('Convo C'); assert.equal(await page.locator('#instruction').inputValue(), 'Manual C draft')
    // A newer accepted click supersedes the delayed older one.
    const older = holdRead('thread-b')
    await click('thread-b'); await older.started
    await click('thread-a'); await title('Convo A'); older.release()
    await page.waitForTimeout(200)
    await title('Convo A'); assert.equal(await page.locator('#instruction').inputValue(), 'Draft A while running')
    await click('missing-thread')
    await page.getByText('Cuộc hội thoại trong thông báo không còn khả dụng.', { exact: false }).waitFor()
    await title('Convo A')
    assert.equal(await page.locator('#instruction').inputValue(), 'Draft A while running')
    // Revocation while lookup is awaiting native RPC must not select/resume its target.
    const beforeLock = holdRead('thread-b')
    await click('thread-b'); await beforeLock.started
    if (viewport.width < 760) await page.getByRole('button', { name: 'Open conversations', exact: true }).click()
    await page.getByLabel('Settings', { exact: true }).click()
    await page.getByRole('button', { name: 'Lock app', exact: true }).click()
    await page.getByLabel('Khóa mã hóa riêng', { exact: true }).waitFor()
    const afterLockCalls = nativeCalls.length
    beforeLock.release(); await page.waitForTimeout(200)
    assert.equal(await page.locator('#instruction').count(), 0)
    assert.equal(nativeCalls.slice(afterLockCalls).filter(call => call.method === 'thread/resume').length, 0)
    // The pre-Lock request may be cancelled/consumed; a fresh accepted click
    // after a new unlock must still route correctly without stale interference.
    await login(); await unlock(); await click('thread-b'); await title('Convo B')
  ...
  console.log(JSON.stringify({ passed: true, viewports: [1280, 390], manualSelectionWins: true, newerNotificationWins: true, delayedLockNoResume: true, coldTargetAfterUnlock: true, lockedClickDeferred: true, encryptedThreadApi: true, inPlaceRouting: true, draftsPreserved: true, activeTurnNotInterrupted: true, missingTargetFallback: true, realPushSent: false, realModelTurns: 0, physicalIOS: false, screenshots }))
} finally {

...
const hostBefore = await hostNginxIdentity()
...
try {
  ...
  nginx = await startNginxFixture(`server { listen 127.0.0.1:${port}; server_name 127.0.0.1;
...
location / { proxy_pass http://127.0.0.1:${config.port}; proxy_set_header Host $http_host; }
  ...
  assert(ready, 'fixture Nginx must start'); await nginx.assertIdentity()
  ...
  assert.deepEqual(await hostNginxIdentity(), hostBefore, 'Host Nginx owners/modes/inodes and service/process identity must not change')
  console.log(JSON.stringify({ nginxNonRootRestrictedFilesystem: true, allFiveTempPathsIsolated: true, hostNginxUnchanged: true }))
}

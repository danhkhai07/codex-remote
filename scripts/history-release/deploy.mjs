...
function prepare(release, artifacts, evidencePath) {
  ...
  const manifest = { version: 1, status: 'prepared-pending-fresh-baseline-and-review', source: git(worktree, 'rev-parse', 'HEAD'), worktree,
    ...
    pending: ['Leader review of history paging/cache/native parity and runner', 'Fresh ALL-idle; no own-task exemptions'] }
...
async function adapter(release) {
  ...
  return { preflight: async () => assertSnapshot(false), assertBaseline: () => assertSnapshot(false), assertPublished: () => assertSnapshot(true), readiness, backup, publish, state, sleep,
    acquirePublication: () => { unlock = publicationLock('/root/.local/state/codex-remote/deployment.lock', { kind: 'NEW-history', pid: process.pid, release, source: m.source }) },
...
async function main() {
  ...
  assert(dirname(release) === S + '/releases' && release.split('/').at(-1).startsWith('history-window-'), 'Expected private NEW release path')

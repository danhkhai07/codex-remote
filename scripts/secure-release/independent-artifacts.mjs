...
for (const name of ['common.mjs', 'production.mjs', 'runner.mjs', 'verify.mjs', 'destinations.mjs', 'publication.mjs', 'lock.mjs', 'key-state.mjs', 'key-cutover.mjs', 'cutover-ops.mjs', 'key-init.mjs', 'cutover-bin/systemctl']) assert.equal(readFileSync(join(release, 'runner', name), 'utf8'), git('show', meta.runnerSource + ':scripts/secure-release/' + name))

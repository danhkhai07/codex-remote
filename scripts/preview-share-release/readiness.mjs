...
// Deliberately conservative: systemError/unknown status stays busy. Never request
// a thread transcript to reinterpret it as idle. Pagination/incomplete is closed.
export async function metadataReadiness(api) {
  const threads = await api('/api/threads'), pending = await api('/api/pending')
  assert(Array.isArray(threads?.data) && Array.isArray(pending?.data), 'Incomplete readiness metadata')
  const busy = threads.data.filter(t => !['idle', 'notLoaded'].includes(t?.status?.type)).length
  return { ready: !threads.nextCursor && busy === 0 && pending.data.length === 0, busy, pending: pending.data.length, incomplete: Boolean(threads.nextCursor) }
}

...
export function atomic(path, bytes, mode = 0o600) {
  ...
  const temp = path + '.owner-files-' + process.pid
...
export function ownerEnv(original) {
  const before = parseEnv(original)
  assert.equal(before.CODEX_REMOTE_PUBLIC_ORIGIN, 'https://remote.danhkhai.io.vn')
  assert.equal(before.CODEX_REMOTE_PORT, '5174'); assert.equal(before.CODEX_REMOTE_SECURE_API, 'required')
  const desired = { CODEX_REMOTE_FILE_ACCESS: 'owner-full', CODEX_REMOTE_FILE_ROOTS: '/' }
  const lines = original.split('\n').filter(line => !/^\s*(?:export\s+)?CODEX_REMOTE_FILE_(?:ACCESS|ROOTS)\s*=/.test(line))
  const updated = lines.join('\n').replace(/\n*$/, '\n') + Object.entries(desired).map(([k, v]) => `${k}=${JSON.stringify(v)}`).join('\n') + '\n'
  same(parseEnv(updated), { ...before, ...desired }, 'Unrelated env values')
  return updated
}

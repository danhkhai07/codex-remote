...
- On the shared VPS, run builds, tests, type checks, scans and browser/rendering checks with `codex-heavy --label <task-check> -- <command>` (or `python3 scripts/codex-heavy.py ...` if not installed). This queues heavy jobs and runs them in a separate, resource-limited systemd service. Keep checks within the job sequential; do not bypass the runner after OOM/timeout. See `docs/bounded-workloads.md`. Small reads/edits/Git operations do not need it; the runner's own bounded fixture test runs directly.

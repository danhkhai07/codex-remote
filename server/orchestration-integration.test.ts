...
function setup() {
  ...
  const rpc = vi.spyOn(app, 'request').mockImplementation(async (method, input) => {
    ...
    if (method === 'model/list') return { data: [{ model: 'gpt-6-astra', supportedReasoningEfforts: [{ reasoningEffort: 'high' }] }] }
    return {}
  })
...
it('keeps leader changes and team controls behind session, CSRF and group checks', async () => {
  ...
  const cap = orchestra.context('leader', { model: 'gpt-6-astra', effort: 'high', fullAccess: false }, true).match(/--capability ([\w-]+)/)![1]

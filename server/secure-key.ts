import type { FileAccess } from './file-policy.js'
...
export function assertKeyLocation(file: string, roots: string[], mode: FileAccess = 'restricted') { if (!isAbsolute(file) || (mode !== 'owner-full' && insideRoots(resolve(file), roots))) throw Error('Secure key must be outside allowed file roots') }
/** Never include secret contents in exceptions/logs. Reload allows local rotation. */
export function readOwnerKey(file: string, roots: string[], mode: FileAccess = 'restricted'): OwnerKeyFile {
  assertKeyLocation(file, roots, mode)
  let fd: number | undefined
  try {
    fd = openSync(file, constants.O_RDONLY | constants.O_NOFOLLOW | constants.O_NONBLOCK)
    const info = fstatSync(fd), canonical = realpathSync(`/proc/self/fd/${fd}`)
    if (!info.isFile() || (info.mode & 0o777) !== 0o600 || info.uid !== process.getuid?.() || info.size > 2048 || (mode !== 'owner-full' && insideRoots(canonical, roots))) throw Error()

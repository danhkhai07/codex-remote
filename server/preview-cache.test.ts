...
async function fixture() {
  ...
  const host = 'p' + port + '.test'
  proxy = new LocalhostPreview({ originTemplate: 'http://p{port}.test', sessionSecret: 'fixture-only', blockedPorts: [gatewayPort] })
  ...
  async function get(path: string, headers: Record<string, string> = {}, method = 'GET') {
    return new Promise<{ status: number; headers: IncomingMessage['headers']; body: string }>((resolve, reject) => {
      const req = request({ hostname: '127.0.0.1', port: gatewayPort, path, method, headers: { host, ...headers } }, res => {
  ...
  const launch = new URL(proxy.createLaunch(port, '/', createSession('fixture-only', 600).payload).url)
  const cookie = (await get(launch.pathname + launch.search)).headers['set-cookie']![0].split(';')[0]
  return { get, cookie, port, received, change: () => version++ }
}

it('revalidates current delivered bytes, ports and HEAD correctly on isolated origins', async () => {
...
it('keeps private assets revalidatable and independently validates each port', async () => {
  ...
  expect(other.body).toContain('version = 2')
})

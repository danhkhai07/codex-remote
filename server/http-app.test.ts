...
describe('Codex Remote HTTP boundary', () => {
  ...
  it('serves the login shell publicly while protecting APIs and login origin', async () => {
    ...
    const messageIds = vi.spyOn(controller, 'readMessageIds').mockResolvedValue({ ids: ['turn:reply'], nextCursor: null })
    ...
    expect(JSON.parse(messageSummary.body)).toEqual({ ids: ['turn:reply'], nextCursor: null })
    expect(messageIds).toHaveBeenCalledWith('chat', expect.any(Function), undefined, { initialized: false, known: new Set() })
    ...
    messageIds.mockResolvedValue({ ids: ['turn:reply', 'reply:finished'], nextCursor: null })
    ...
    messageIds.mockResolvedValue({ ids: ['turn:reply', 'reply:finished', 'reply:newer'], nextCursor: null })

...
import { allowedFilePath as allowed, type FileAccess } from './file-policy.js'

export async function listDirectory(query: URLSearchParams, roots: string[], mode: FileAccess = 'restricted') {
  ...
  if (!allowed(input, roots, mode)) throw new ServerFileError(403, 'Directory access is not allowed')
  let directory: FileHandle | undefined
  try {
    const path = await realpath(resolve(input))
    if (!allowed(path, roots, mode)) throw new ServerFileError(403, 'Directory is outside the configured file roots')
    ...
    if (!allowed(await realpath(descriptor), roots, mode)) throw new ServerFileError(403, 'Directory access is not allowed')
    ...
    const children = (await readdir(descriptor, { withFileTypes: true }))
      .filter(entry => allowed(join(path, entry.name), roots, mode) && (hidden || !entry.name.startsWith('.')) && (!search || entry.name.toLocaleLowerCase().includes(search)))
    ...
    const entries = await Promise.all(children.slice(offset, offset + limit).map(async entry => {
      ...
      try {
        ...
        if (!allowed(target, roots, mode)) return { ...base, kind: 'unavailable' as const, size: null, modifiedAt: null }
        const type = await stat(target)
        if (!type.isFile() && !type.isDirectory()) return { ...base, kind: 'unavailable' as const, size: null, modifiedAt: null }
        const handle = await open(target, constants.O_RDONLY | constants.O_NOFOLLOW | constants.O_NONBLOCK)
        try {
          if (!allowed(await realpath(`/proc/self/fd/${handle.fd}`), roots, mode)) return { ...base, kind: 'unavailable' as const, size: null, modifiedAt: null }
    ...
    return { path, parentPath: parent !== path && allowed(parent, roots, mode) ? parent : null, entries, total: children.length, offset, limit }
  } catch (error) {

...
export async function unseal(key: SecureKey, context: FrameContext, packet: string): Promise<Uint8Array> {
  ...
  const { protectedHeader: h, plaintext } = await compactDecrypt(packet, key, { keyManagementAlgorithms: ['dir'], contentEncryptionAlgorithms: ['A256GCM'] })
...
export async function* wireLines(source: AsyncIterable<Uint8Array>): AsyncGenerator<string> {
  let pending = ''
